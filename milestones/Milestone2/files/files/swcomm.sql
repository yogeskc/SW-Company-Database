...
CREATE TABLE IF NOT EXISTS `User` (
  ...
  `fname` VARCHAR(45) NOT NULL DEFAULT 'CONCAT(name,\" \",middlename,\" \",l',
  `middlename` VARCHAR(45) NOT NULL,
  `addr_id` TINYINT UNSIGNED NULL,
  `lname` VARCHAR(45) NOT NULL,
  `company_id` TINYINT UNSIGNED NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `Credentials` (
  ...
  INDEX `FK_acc_id_idx` (`acc_id` ASC) VISIBLE,
  PRIMARY KEY (`acc_id`),
  CONSTRAINT `FK_acc_cr_id`
    FOREIGN KEY (`acc_id`)
...
CREATE TABLE IF NOT EXISTS `PaymentInfo` (
  `payment_id` TINYINT UNSIGNED NULL,
  UNIQUE INDEX `payment_id_UNIQUE` (`payment_id` ASC),
  CONSTRAINT `FK_process_payment`
    FOREIGN KEY (`payment_id`)
    REFERENCES `ProcessPayment` (`device_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
